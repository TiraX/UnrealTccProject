...
void UTctrBigmidTree::UpdateParameters() 
{
	...
	}
	{
		...
		_tct_directional_warp1->Dir = FVector2f(hs_cos(_tct_directional_warp1->Angle), hs_sin(_tct_directional_warp1->Angle));
	...
	}
	{
		...
		_multiply->M = FVector4f(hs_cos( - _multiply->R) / _multiply->Sx, hs_sin( - _multiply->R) / _multiply->Sy, -hs_sin( - _multiply->R) / _multiply->Sx, hs_cos( - _multiply->R) / _multiply->Sy);
	...
	}
	{
		...
		_multiply1->M = FVector4f(hs_cos( - _multiply1->R) / _multiply1->Sx, hs_sin( - _multiply1->R) / _multiply1->Sy, -hs_sin( - _multiply1->R) / _multiply1->Sx, hs_cos( - _multiply1->R) / _multiply1->Sy);
	...
	}
	{
		...
		_tct_blend1->M = FVector4f(hs_cos( - _tct_blend1->R) / _tct_blend1->Sx, hs_sin( - _tct_blend1->R) / _tct_blend1->Sy, -hs_sin( - _tct_blend1->R) / _tct_blend1->Sx, hs_cos( - _tct_blend1->R) / _tct_blend1->Sy);
	...
	}
	{
		...
		float _bottom = 0.900000f;
		_blue_channel->UpdateHelperNodeParameters();
	}
	{
