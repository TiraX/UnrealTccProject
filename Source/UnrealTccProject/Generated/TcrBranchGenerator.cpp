...
FTccNodePtr UTcrBranchGenerator::CreateNode() 
{
	TSharedPtr<FTcrBranchGenerator> Node = MakeShared<FTcrBranchGenerator>();
	SyncParams(Node);
	return Node;
}
void UTcrBranchGenerator::SyncParams(FTccNodePtr InNode) 
{
	TSharedPtr<FTcrBranchGenerator> Node = StaticCastSharedPtr<FTcrBranchGenerator>(InNode);
 ...
 FTcrBranchGenerator::FTcrBranchGenerator() 
{
	InitInputsCount(1);
	...
	need_bend1 = new FTccSwitch();
	need_bend1->InitMultiRefs(false); // RefCount = 1
		tcc_bend1 = new FTccBend();
		tcc_bend1->InitMultiRefs(false); // RefCount = 1
	need_bend2 = new FTccSwitch();
	need_bend2->InitMultiRefs(true); // RefCount = 999
...
void FTcrBranchGenerator::Cook() 
{
	...
	}
	{
		...
		need_bend1->Input = int32(Bend1);
		need_bend1->NumCases = 2;
		need_bend1->Cook();
		FTccGeometryPtr SwitchResult = nullptr;
		const int32 Selection = need_bend1->Input;
		switch (Selection)
		{
			case 0:
			{
	...
	}
	{
		...
		need_bend2->Input = int32(Bend2);
		need_bend2->NumCases = 2;
		need_bend2->Cook();
		FTccGeometryPtr SwitchResult = nullptr;
		const int32 Selection = need_bend2->Input;
		switch (Selection)
		{
			case 0:
			{
	...
	}
	{
		...
		foreach_branch->Cook();
		{
			...
			for(int32 piece = 0; piece < NumPieces; piece ++)
			{
				...
				FTccNode* block_input1 = need_bend2;
				{
	...
	{
		tcc_merge1->SetInput(0, GetInput(0));
	...
	SetGeoResult(UTcrBranchGenerator::output0, inc_curr_level->GetGeoResult(0));
}
